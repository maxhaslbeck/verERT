...
| "wp (Par c\<^sub>1 c\<^sub>2) f    = wp c\<^sub>1 f \<squnion> wp c\<^sub>2 f"
...
lemma wp_mono: "mono (wp c)"
  by (induction c)
     (auto simp: mono_def le_fun_def intro: order_trans sup_mono le_supI1 le_supI2
...
proof (induction arbitrary: s y rule: lfp_ordinal_induct[OF mono_r'[where f=f]])
  case (1 F) with *[of s y F] show ?case
    unfolding le_fun_def E_sup_r[where f=f, symmetric] by simp
qed (auto intro: SUP_least)

lemma E_sup_Skip: "step.E_sup (Skip, s) (r f) = f s"
  by (subst E_sup_r_unfold) simp

lemma E_sup_Seq:
  assumes [simp]: "\<And>x. 0 \<le> f x"
  shows "step.E_sup (Seq a b, s) (r f) = step.E_sup (a, s) (r (\<lambda>s. step.E_sup (b, s) (r f)))"
proof (rule antisym)
  show "step.E_sup (Seq a b, s) (r f) \<le> step.E_sup (a, s) (r (\<lambda>s. step.E_sup (b, s) (r f)))"
...
lemma E_sup_While:
  "step.E_sup (While g c, s) (r f) =
    lfp (\<lambda>F s. if g s then step.E_sup (c, s) (r F) else f s) s"
proof (rule antisym)
  have E_sup_While_step: "step.E_sup (While g c, s) (r f) =
    (if g s then step.E_sup (c, s) (r (\<lambda>s. step.E_sup (While g c, s) (r f))) else f s)" for f s
    by (rewrite E_sup_r_unfold) (simp add: min_absorb1 E_sup_Seq)

  have "mono (\<lambda>F s. if g s then step.E_sup (c, s) (r F) else f s)" (is "mono ?F")
    by (auto intro!: mono_r step.E_sup_mono simp: mono_def le_fun_def max.coboundedI2)
  then show "lfp ?F s \<le> step.E_sup (While g c, s) (r f)"
  ...
  proof (coinduction arbitrary: t d s rule: E_sup_r_induct)
    case (step F t d s)
    from step(1)
    show ?case
    proof (elim conjE disjE)
      { fix s have "\<not> g s \<Longrightarrow> F (While g c, s) \<le> f s"
          using step(3)[of "(While g c, s)"] by (simp add: E_sup_While_step) }
      note [simp] = this
  ...
  also have "lfp w = lfp (\<lambda>F s. step.E_sup s (r (\<lambda>s. if g s then F (c, s) else f s)))"
    unfolding E_sup_r w_def
    by (rule lfp_lfp[symmetric]) (auto simp: le_fun_def intro!: SUP_mono[OF bexI] nn_integral_mono)
  finally have "step.E_sup (While g c, s) (r f) \<le> (if g s then \<dots> (c, s) else f s)"
    unfolding t_def d_def by (rewrite E_sup_r_unfold) simp
  also have "\<dots> = lfp ?F s"
    by (rewrite lfp_rolling[symmetric, of "\<lambda>F s. if g s then F (c, s) else f s"  "\<lambda>F s. step.E_sup s (r F)"])
       (auto simp: mono_def le_fun_def sup_apply[abs_def] if_distrib[of "max 0"] max.coboundedI2 max.absorb2
                intro!: step.E_sup_mono mono_r cong del: if_cong)
  finally show "step.E_sup (While g c, s) (r f) \<le> \<dots>"
    .
qed

lemma E_sup_r_eq_wp: "step.E_sup (c, s) (r f) = wp c f s"
proof (induction c arbitrary: f s)
  case Skip then show ?case
    by (simp add: E_sup_Skip)
next
  ...
  proof (intro antisym)
    ...
    then show "step.E_sup (Abort, s) (r f) \<le> wp Abort f s"
      by (auto simp: E_sup_r le_fun_def split: if_split_asm)
  qed simp
next
  case Assign then show ?case
    by (rewrite E_sup_r_unfold) (simp add: min_absorb1)
next
  case (If b c1 c2) then show ?case
    by (rewrite E_sup_r_unfold) auto
next
  case (Prob p c1 c2) then show ?case
    apply (rewrite E_sup_r_unfold)
    apply auto
...
next
  case (Par c1 c2) then show ?case
    by (rewrite E_sup_r_unfold) (auto intro: sup.commute)
next
  case (Seq c1 c2) then show ?case
    by (simp add: E_sup_Seq)
next
  case (While g c) then show ?case
    apply (simp add: E_sup_While)
    apply (rewrite While)
    apply auto
    done
qed
