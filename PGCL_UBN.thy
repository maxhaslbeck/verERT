...
| nPar: "(\<And>i. halt_free (c i)) \<Longrightarrow> halt_free (nPar c)"
...
lemma sup_continuous_ert: "sup_continuous (ert P)"
  ...
  subgoal (* I think this is not correct *)
    sorry
  done
...
next
  ...
  proof (intro lfp_greatest)
    ...
    also have "(SUP n::nat. (\<lambda>_. min k n)) = (\<lambda>x. k)"
      by (auto intro!: antisym SUP_least le_funI simp: SUP_image inf_min[symmetric] inf_SUP[symmetric] ennreal_SUP_of_nat_eq_top)
